...
df = pd.read_csv(r'C:\Users\dhana\PycharmProjects\DataAnalyticsProject\venv\athlete_events.csv')
region_df = pd.read_csv(r'C:\Users\dhana\PycharmProjects\DataAnalyticsProject\venv\noc_regions.csv')
...
if user_menu =='Medal Tally':
	...
	if selected_year != 'Overall' and selected_country == 'Overall':
		st.title(f'Medal Tally in {int(selected_year)} Olympics')
...
if user_menu == 'Overall Analysis':
	...
	st.title('Participating Nations in over the Years')
	st.plotly_chart(fig)
	...
	st.title('Most Successfull Players')
	sport_list = df['Sport'].unique().tolist()
	sport_list.sort()
	sport_list.insert(0,'Overall')
	selected_sport = st.selectbox('Select a Sport', sport_list)
	sp = helper.most_successful(df,selected_sport)
	st.table(sp)
...
if user_menu == 'Country-wise Analysis':
	...
	st.title(f'Top 10 Athletes for {selected_country}')
